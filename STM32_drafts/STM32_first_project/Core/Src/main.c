...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  if (GPIO_Pin == GPIO_PIN_1 && (currentMillis - previousMillis > 25))
  {
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if(htim -> Instance == TIM2)
    {
        ...
        Temp = (Value2 - Value1) / 59; // * 0.034 / 2
        if(Temp < 0 || Temp > 400) return;
        Distance = Temp;

        if(Flag) {
            tm1637DisplayDecimal(Distance - Saved);
        } else {
            tm1637DisplayDecimal(Distance);
        }  
    }
}
